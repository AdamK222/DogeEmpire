...
contract ERC20DividendToken is Context, ERC20, Ownable {
    ...
    function _deployDividendTracker(address _uniswapV2Router, address rewardToken_) internal {
        ...
        if (marketingAddress != owner()) dividendTracker.excludeFromDividends(marketingAddress);
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        uint256 portionForMarketing = dividends.mul(3).div(1000); // 0.3% of 5% for marketing

        dividends = dividends.sub(portionForMarketing);
        bool success = IERC20(rewardToken).approve(address(dividendTracker), dividends);

        if (getNumberOfDividendTokenHolders() > 0 && success) {
            IERC20(rewardToken).transfer(marketingAddress, portionForMarketing);
...
contract ERC20DividendTracker is Ownable, DividendPayingToken {
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "Dividend_Tracker: claimWait must be updated to between 1 minute and 24 hours"
        );
        require(
...
contract DEMP is ERC20DividendToken {
    ...
    address private _rewardToken = 0xbA2aE424d960c26247Dd6c32edC70B295c744C43;

    constructor (address _marketingAddress) ERC20DividendToken("DOGE EMPIRE", "DEMP", _tokenSupply, _routerAddress, _rewardToken, _marketingAddress) {
